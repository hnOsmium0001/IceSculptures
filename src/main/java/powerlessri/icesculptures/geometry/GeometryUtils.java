...
public final class GeometryUtils {
    ...
    public static final int[][] triangulations = new int[][] {
            new int[] {},
            new int[] {0, 8, 3},
            new int[] {0, 1, 9},
            new int[] {1, 8, 3, 9, 8, 1},
    ...
    public static int[] cornerIdxA = new int[] {0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3};

    public static int[] cornerIdxB = new int[] {1, 2, 3, 0, 5, 6, 7, 4, 4, 5, 6, 7};

    // Segments per axis
    public static final int spa = 16;
    // Points per axis
    public static final int ppa = 17;
    // Segment length
    public static final float sl = 1F / spa;

    // Cache for vertices in a block to help with small object allocations
    public static final Vector3f[] verts = new Vector3f[ppa * ppa * ppa];

    static {
        // Ranging from 0 to 16 inclusively
        for (int x = 0; x < ppa; x++) {
            for (int y = 0; y < ppa; y++) {
                for (int z = 0; z < ppa; z++) {
                    verts[indexFromPos(x, y, z)] = new Vector3f(x * sl, y * sl, z * sl);
                }
            }
        }
    }

    public static Vector3f vertexFromPos(int x, int y, int z) {
        return verts[indexFromPos(x, y, z)];
    }

    public static int indexFromPos(int x, int y, int z) {
        return z * ppa * ppa + y * ppa + x;
    }

    public static int flagFromPos(BitSet lattice, int n, int x, int y, int z) {
        return (lattice.get(indexFromPos(x, y, z)) ? 1 : 0) << n;
    }

    // Adapted from https://github.com/SebLague/Marching-Cubes/blob/master/Assets/Scripts/Compute/MarchingCubes.compute
    // x, y, z here are indices (referred to as `pos` in code) of the vertices in the sculpture block
    public static void marchCube(List<Triangle> triangles, BitSet lattice, int x, int y, int z) {
        // Note: same vertex offset layout (adding to xyz values) for computing corners and triangulation index
        Vector3f[] corners = new Vector3f[] {
                vertexFromPos(x, y, z),
                vertexFromPos(x + 1, y, z),
                vertexFromPos(x + 1, y, z + 1),
                vertexFromPos(x, y, z + 1),
                vertexFromPos(x, y + 1, z),
                vertexFromPos(x + 1, y + 1, z),
                vertexFromPos(x + 1, y + 1, z + 1),
                vertexFromPos(x, y + 1, z + 1)
        };
        int triangulationIdx = flagFromPos(lattice, 0, x, y, z) |
                flagFromPos(lattice, 1, x + 1, y, z) |
                flagFromPos(lattice, 2, x + 1, y, z + 1) |
                flagFromPos(lattice, 3, x, y, z + 1) |
                flagFromPos(lattice, 4, x, y + 1, z) |
                flagFromPos(lattice, 5, x + 1, y + 1, z) |
                flagFromPos(lattice, 6, x + 1, y + 1, z + 1) |
                flagFromPos(lattice, 7, x, y + 1, z + 1);
    ...
    public static Vector3f midpointOf(Vector3f v1, Vector3f v2) {
        return new Vector3f((v1.x + v2.x) / 2, (v1.y + v2.y) / 2, (v1.z + v2.z) / 2);
    }
}